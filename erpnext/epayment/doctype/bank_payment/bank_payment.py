...
class BankPayment(Document):
	...
	if TYPE_CHECKING:
		...
		transaction_type: DF.Literal["", "Salary", "Journal Entry", "Payment Entry", "Process MR Payment", "Bulk Leave Encashment", "Employee Loan Payment", "Employee PF Payment"]
	...
	def get_loan_detail(self):
		...
		if doc.employee_loan_payment_in_bob_account:
			return frappe.db.sql(
					...
					"Draft" status FROM `tabSalary Structure` t1 JOIN `tabSalary Detail` t2 ON t1.name = t2.parent 
...
def get_paid_from(doctype, txt, searchfield, start, page_len, filters):
	...
	data = []
	data = frappe.db.sql(
		"""select a.name, a.bank_name, a.bank_branch, a.bank_account_type, a.bank_ac_no
		from `tabAccount` a
