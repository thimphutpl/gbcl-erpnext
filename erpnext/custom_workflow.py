...
class CustomWorkflow:
	def __init__(self, doc):
		...
		if self.doc.doctype == "Leave Application":
			self.reports_to = frappe.db.get_value("Employee", {"user_id":frappe.db.get_value("Employee", self.doc.employee, "leave_approver")}, self.field_list)
			self.hr_approver	= frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "hr_manager"), self.field_list)
		if self.doc.doctype in ("Travel Authorization", "Travel Advance", "Travel Adjustment", "Travel Claim", "Employee Advance", "Leave Encashment"):
			...
			self.general_manager = frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "general_manager"), self.field_list)
			if not self.general_manager:
				frappe.throw("Plesse set General Manager in HR Settings")

			self.finanace_manager = frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "accounts_finance"), self.field_list)
			if not self.finanace_manager:
				frappe.throw("Plesse set Accounts and Finance approver in HR Settings")
		### =============== *** =============== *** === NYUTHYUE === *** =============== *** =============== ###

		elif self.doc.doctype != "Material Request" and self.doc.doctype not in ("Asset Issue Details", "Compile Budget","POL Expense","Vehicle Request", "Repair And Services", "Asset Movement", "Budget Reappropiation","Leave Application"):
			self.employee		= frappe.db.get_value("Employee", self.doc.employee, self.field_list)
			self.reports_to = frappe.db.get_value("Employee", {"name":frappe.db.get_value("Employee", self.doc.employee, "reports_to")}, self.field_list)
			...
			self.hr_approver	= frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "hr_manager"), self.field_list)
			self.hrgm = frappe.db.get_value("Employee",frappe.db.get_single_value("HR Settings","general_manager"), self.field_list)
			...
			if self.doc.doctype in ["POL","Vehicle Request"]:
		...
		if self.doc.doctype == "Material Request":
			...
			# self.reports_to = frappe.db.get_value("Employee", {"name":frappe.db.get_value("Employee", self.doc.employee, "reports_to")}, self.field_list)
			self.procurement_approval = frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "procurement"), self.field_list)
			if not self.procurement_approval:
				frappe.throw("Plesse set Procurement approver in HR Settings")
	...
	def set_approver(self, approver_type):
		if approver_type == "Supervisor":
			if not self.reports_to:
				frappe.throw("Reports To / Leave Approver not set for Employee {}".format(self.doc.employee if self.doc.employee else frappe.db.get_value("Employee", {"user_id",self.doc.owner}, "name")))
			officiating = get_officiating_employee(self.reports_to[3])
			# frappe.throw(frappe.as_json(self.reports_to))
			if officiating:
		...
		elif approver_type =="Procurement Approval":
			officiating = get_officiating_employee(self.procurement_approval[3])
			if officiating:
				officiating = frappe.db.get_value("Employee", officiating[0].officiating_employee, self.field_list)
			vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.procurement_approval[0]
			vars(self.doc)[self.doc_approver[1]] = officiating[1] if officiating else self.procurement_approval[1]
			vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.procurement_approval[2]	
		...
		elif approver_type == "Finance Manager":
			officiating = get_officiating_employee(self.finanace_manager[3])
			if officiating:
				officiating = frappe.db.get_value("Employee", officiating[0].officiating_employee, self.field_list)
			vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.finanace_manager[0]
			vars(self.doc)[self.doc_approver[1]] = officiating[1] if officiating else self.finanace_manager[1]
			vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.finanace_manager[2]
	...
	def leave_application(self):
		...
		# 	self.set_approver("Supervisor")
			...
			if frappe.session.user != self.doc.leave_approver:
				frappe.throw("Only {} can forward this leave".format(self.doc.leave_approver))
	...
	def travel_claim(self):
		...
		elif self.new_state.lower() == ("Waiting Finance Approval".lower()):
			if frappe.session.user != self.doc.approver:
				frappe.throw(f"Only {self.doc.approver} can Forward this Request.")
			self.set_approver("Finance Manager")
