...
def test_connectivity(bank):
    if not frappe.db.exists('Bank Payment Settings', bank):
        frappe.throw(_("Bank Payment Settings entry not found for: {0}").format(bank))

    try:
        sftp = SftpClient(bank)
        sftp.close()
        frappe.msgprint(_('Connection successful to {0}').format(bank))
        return {'status': 'success', 'message': f'Connected to {bank}'}

    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Bank SFTP Connectivity Error")
        frappe.throw(_("Failed to connect to {0}: {1}").format(bank, str(e)))
