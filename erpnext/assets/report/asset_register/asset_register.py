...
def get_data(filters):
    query = """
        SELECT
            a.name, a.asset_name, a.asset_category, a.asset_sub_category, a.company,
            a.vehicle_number, a.serial_number, a.old_asset_code,
            a.cost_center, a.purchase_date, a.purchase_date as date_of_issue, a.status, a.asset_status, 
            a.disposal_date, a.journal_entry_for_scrap,
            a.custodian as issued_to, a.custodian_name as employee_name,
    ...
    if filters.cost_center:
        query += " AND a.cost_center = %(cost_center)s"

    if filters.asset_category:
        query += " AND a.asset_category = %(asset_category)s"
...
def get_columns():
    return [
        ...
        # {
        #     "fieldname": "dep_total_next_year",
        #     "label": _("Next Year Depreciation"),
        #     "fieldtype": "Currency",
        #     "width": 120
        # },
    ]