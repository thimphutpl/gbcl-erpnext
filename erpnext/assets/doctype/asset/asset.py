...
# # 		employee  = frappe.db.sql("SELECT name FROM `tabEmployee` where user_id='{}' ".format(user) )
	

# # 	if user == "Administrator":
# # 		return
# # 	# if "Accounts User" in user_roles or "Accounts Master" in user_roles or "Accounts Manager":
# # 	# 	return
# # 	return """(
# # 		`tabAsset`.custodian = '{employee}' 
# # 		""".format(employee=employee[0])
# def get_permission_query_conditions(user):
#     if not user:
...
#     # Accounts roles can see everything
#     if any(role in user_roles for role in ["Accounts User", "Accounts Master", "Accounts Manager"]):
#         return ""

#     # Get employee linked to user
#     employee = frappe.db.get_value("Employee", {"user_id": user}, "name")

#     # If no employee is linked, show nothing
#     if not employee:
#         return "1=0"
...
class Asset(AccountsController):
	...
	def validate_asset_finance_books(self, row):
		...
		if row.depreciation_start_date and getdate(row.depreciation_start_date) < getdate(self.purchase_date):
			frappe.throw(
				_("Depreciation Row {0}: Next Depreciation Date cannot be before Purchase Date.{1}-{2}").format(
					row.idx,row.depreciation_start_date,self.purchase_date
				)
			)
